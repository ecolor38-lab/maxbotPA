...
class NewsCollector {
  ...
  async run() {
    ...
    try {
      ...
      // Проверяем минимальное количество постов (12 на 3 дня)
      const minPosts = config.search.minPostsInPlan || 12;
      if (stats.pending < minPosts) {
        console.log(`\n⚠️ В плане ${stats.pending} постов, нужно минимум ${minPosts} (на 3 дня)`);
        console.log('💡 Рекомендуется собрать еще новостей для накопления контента');
      } else {
        console.log(`\n✅ Контент-план заполнен! Достаточно постов на 3+ дня (${stats.pending}/${minPosts})`);
      }
