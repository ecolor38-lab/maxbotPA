...
services:
  ai-bot:
    ...
    volumes:
      - ./posts:/app/posts
      - ./images:/app/images
      # Не монтируйте файлы напрямую - это может вызвать проблемы с правами
      # Приложение создаст content-plan.json и published-posts.json автоматически
      # Если нужна персистентность, смонтируйте директорию данных:
      # - ./data:/app/data
```

**⚠️ Важно:** Не монтируйте файлы `content-plan.json` или `published-posts.json` напрямую в Docker, так как это может вызвать ошибки прав доступа (`EACCES: permission denied`). Приложение создаст эти файлы автоматически при первом запуске.

Если вам нужна персистентность данных между перезапусками контейнера, рекомендуется:
1. Использовать внешнюю базу данных (PostgreSQL, MongoDB)
2. Или монтировать директорию данных вместо отдельных файлов
